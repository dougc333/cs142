...
class UserPhotos extends React.Component {
  ...
  //{this.addLikes(this.state.photos[i].likes,this.state.photos[i]._id)}
  //<div id="numLikes" className="child">{this.state.photos[i].likes.length}</div>
  addLikes(arrLike,id){
    console.log("addLikes arrLike:", arrLike," this.login_info.userId:",this.state.login_info.userId)
    console.log("addLikes id:",id)
    let arr=[]
    let res = arrLike.filter(x=>{console.log("x.user_id:",x.user_id);console.log("this.state.login_info.user_id",this.state.login_info.userId); console.log(x.user_id===this.state.login_info.userId)})
    console.log("addLikes res:",res)
   
      
    if(res.length==0){
      arr.push(
          <Button onClick={this.handleUnLike} photo_id={id} key={Math.random()}>Like</Button>
      )
       //do setState here! infinite loop if setState
    }else{
      arr.push(
        <Button onClick={this.handleLike} photo_id={id} key={Math.random()}>Unlike</Button>
      )
       //do setState here! infinite loop
    }
    return arr
  }

  handleUnLike(e){
    //we need to do a database update to webserver to increase likes or decrease likes depending on value
    console.log("handleUnLike photoId:",e.currentTarget.getAttribute("photo_id"))

    Axios.get(`http://localhost:3000/Unlike/${e.currentTarget.getAttribute("photo_id")}`)
    .then(response=>{
      console.log("handleUnLike response:",response)
      //delete the div! from teh dom! or hide it. This doesnt owrk. 
      //cached in browser
      if(response.status===200){
        console.log("status 200 Unlike completed!!!!!! ")
      }
      
    })
    .catch(error=>console.log("Compoment UserPhotos Unlike error:",error))
  }

  handleLike(e){
    //we need to do a database update to webserver to increase likes or decrease likes depending on value
    console.log("handleLike photoId:",e.currentTarget.getAttribute("photo_id"))
   
    Axios.get(`http://localhost:3000/likes/${e.currentTarget.getAttribute("photo_id")}`)
    .then(response=>{
      console.log("handleLike response:",response)
      //delete the div! from teh dom! or hide it. This doesnt owrk. 
      //cached in browser
      if(response.status===200){
        console.log("status 200 likes completed!!!!!! ")
      }
      
    })
    .catch(error=>console.log("Compoment UserPhotos handleLike error:",error))
  }
